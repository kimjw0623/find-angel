...
class Item:
    ...
    def __init__(self, name, grade, price, icon, endDate, type, options, gradeQuality=None, tradeAllowCount=None, upgradeLevel=None):
        self.name = name
        self.grade = grade
        self.price = price
        self.icon = icon
    ...
    def isBracelet(self):
        return self.type == 200040
    
    def isAccessory(self):
        return self.type in [200010, 200020, 200030]
        ...
        for op in options:
            if op["Type"] == "ACCESSORY_UPGRADE":
                if op["OptionName"] == "공격력 " and op["IsValuePercentage"]:
                    toReturn.append((3, 54, Item.effect_value_to_code[54][op["Value"]]))
                elif op["OptionName"] == "무기 공격력 " and op["IsValuePercentage"]:
                    toReturn.append((3, 53, Item.effect_value_to_code[53][op["Value"]]))
                else:
                    toReturn.append((3, Item.effect_to_code[op["OptionName"]], Item.effect_value_to_code[Item.effect_to_code[op["OptionName"]]][op["Value"]]))
    ...
    def parse_bracelet_options(options):
        ...
        for op in options:
            if op["Type"] == "STAT":
                toReturn.append((2, Item.stat_to_code[op["OptionName"]], int(op["Value"])))
            elif op["Type"] == "BRACELET_RANDOM_SLOT":
                toReturn.append((4, 2, int(op["Value"])))
            elif op["Type"] == "????????????": #TODO: check if this is correct
                toReturn.append((1, Item.stat_to_code[op["OptionName"]], int(op["Value"])))
            elif op["Type"] == "????????????": # TODO: check if this is correct
    ...
    def from_response_json(json):
        ...
        if type == 200040:
            ...
            return Item(name, grade, price, icon, endDate, type, options)
        else:
            ...
            return Item(name, grade, price, icon, endDate, type, options, gradeQuality, tradeAllowCount, upgradeLevel)
    ...
    def evaluate(self, evaluator):
        ...
        if self.evaluated_price is None:
            self.evaluated_price = -1
            print("Failed to evaluate item price")
            print(self.foramt_single_line_message())

class ItemEvaluator2:
    def __init__(self, price_cache, debug=False):
        self.price_cache = price_cache
        self.cache_updated = self.price_cache.get_last_update_time()

        self.debug = debug

    def evaluate(self, item):
        if item.isBracelet():
            self.evaluate_bracelet(item)
        elif item.isAccessory():
            self.evaluate_accessory(item)
        else:
            raise ValueError("Unknown item type, Item name and type: " + item.name + item.type)
    
    def evaluate_bracelet(self, item):
        fixed_option_count = sum([first != 4
                                  for (first, _, _)
                                  in item.options]) # 4 for BRACELET_RANDOM_SLOT
        extra_option_count = sum([value
                                  for (first, _, value) in item.options
                                  if first == 4]) # 4 for BRACELET_RANDOM_SLOT
        combat_stats = []
        base_stats = []
        special_effects = []
        for first, second, value in item.options:
            if first == 2:
                if second in [15, 16, 18]: # 15 for 치명, 16 for 특화, 18 for 신속
                    combat_stats.append((Item.code_to_stat(second), value))
                elif second <= 5: # 3 for 힘, 4 for 민첩, 5 for 지능
                    base_stats.append((Item.code_to_stat(second), value))
                else: # 6 for 체력, 17 for 제압, 19 for 인내, 20 for 숙련
                    special_effects.append((Item.code_to_effect(second), value))
            else:
                special_effects.append((Item.code_to_effect(second), value))
        ...
        item.evaluated_price = self.price_cache.get_bracelet_price(item.grade, query)

    def evaluate_accessory(self, item):

...
lock_file_path = 'evaluation_functions.lock'
evaluate_functions_file_path = 'evaluation_functions.pkl'

def _acquire_lock():
    while True:
        try:
            with open(lock_file_path, 'x'):  # 파일이 존재하지 않을 때만 생성
                return
...
evaluate_functions_skeleton = {
    ...
    "고대 3연마 서폿 귀걸이": None,
    "고대 2연마 서폿 귀걸이": None,
    # "고대 1연마 서폿 귀걸이": None, # 서폿 전용옵 무의미
    "유물 3연마 딜러 귀걸이": None,
    "유물 2연마 딜러 귀걸이": None,
    "유물 1연마 딜러 귀걸이": None,
    "유물 3연마 서폿 귀걸이": None,
    "유물 2연마 서폿 귀걸이": None,
    # "유물 1연마 서폿 귀걸이": None, # 서폿 전용옵 무의미
    "고대 3연마 딜러 반지": None,
    "고대 2연마 딜러 반지": None,
    "고대 1연마 딜러 반지": None,
    "고대 3연마 서폿 반지": None,
...
def save_evaluate_functions(evaluate_functions):
    _acquire_lock()
    try:
        with open(evaluate_functions_file_path, 'wb') as f:
            pickle.dump(evaluate_functions, f)
    finally:
        _release_lock()

def load_evaluate_functions():
    _acquire_lock()
    try:
        with open(evaluate_functions_file_path, 'rb') as f:
            evaluate_functions = pickle.load(f)
    finally:
        _release_lock()
